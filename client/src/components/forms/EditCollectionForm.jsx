...
const EditCollectionForm = ({ open, onClose, onSubmit, collection }) => {
  ...
  useEffect(() => {
    if (collection) {
      setFormData({
        ...
        date_time: new Date(collection.date_time),
        waste_type: collection.waste_type.toLowerCase(),
        assigned_team: collection.assigned_team,
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch(`http://localhost:8080/api/collections/${collection.id}`, {
        method: 'PUT',
        headers: {
